...
public:
    ...
    explicit PolicyParser(const std::filesystem::path& filename, const int commandsTimeout, const bool remoteEnabled, std::unique_ptr<IYamlDocument> yamlDocument = nullptr);
...
private:
    ...
    /// @brief Flag indicating whether the policy being parsed is remote.
    bool m_isRemote;
